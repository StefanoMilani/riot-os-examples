 ...
 *
 * @file
 * @brief       Demonstrating the sending and receiving of UDP data over POSIX
 * sockets.
 *
...
int manage_key_request(char* src_addr) {
  ...
  udp_send(src_addr, DEFAULT_PORT, server_buffer,
           public_key_size + MSG_TYPE_SIZE, DEFAULT_NUM, DEFAULT_DELAY);
  printf("KEY SENT\n");
...
int send_ack(char* src_addr) {
  ...
  // Send ACK
  printf("\n-SENDING ACK TO %s.\n", src_addr);
  udp_send(src_addr, DEFAULT_PORT, server_buffer, MSG_TYPE_SIZE, DEFAULT_NUM,
           DEFAULT_DELAY);
  printf("ACK SENT\n");
...
int decrypt_received_message(char* src_addr, int data_count) {
  ...
  if (cipher_init(&aes_context, CIPHER_AES, (const uint8_t*)dev->secret,
                  AES_KEY_SIZE_128) != CIPHER_INIT_SUCCESS) {
  ...
  ret =
      cipher_decrypt_cbc(&aes_context, initialization_vector,
...
int start_exchange(int argc, char** argv) {
  ...
  udp_send(argv[1], DEFAULT_PORT, buf, public_key_size + MSG_TYPE_SIZE,
           DEFAULT_NUM, DEFAULT_DELAY);
  return 0;
}
...
int send_encrypted(int argc, char** argv) {
  ...
  // Initialize context
  int ret = cipher_init(&aes_context, CIPHER_AES, (const uint8_t*)dev->secret,
                        AES_KEY_SIZE_128);
  ...
  size_t plaintext_len =
      (msg_len + 1) + (AES_BLOCK_SIZE - ((msg_len + 1) % AES_BLOCK_SIZE));
  ...
  ret =
      cipher_encrypt_cbc(&aes_context, initialization_vector,
                         (const uint8_t*)plaintext, plaintext_len, ciphertext);
  if (ret < 0) {
    perror("Failed to encrypt data");
    return -1;
  }
  // cipher_encrypt((const cipher_t*)&aes_context, (const uint8_t *) argv[2],
  // (uint8_t*) server_buffer+MSG_TYPE_SIZE);
  memcpy(server_buffer + MSG_TYPE_SIZE + IV_SIZE, ciphertext, plaintext_len);

  // Send encrypted message
  udp_send(argv[1], DEFAULT_PORT, server_buffer,
           MSG_TYPE_SIZE + IV_SIZE + plaintext_len, DEFAULT_NUM, DEFAULT_DELAY);
...
int udp_start_server(char* port_str) {
  ...
  if (thread_create(server_stack, sizeof(server_stack),
                    THREAD_PRIORITY_MAIN - 1, THREAD_CREATE_STACKTEST,
                    _server_thread, port_str,
