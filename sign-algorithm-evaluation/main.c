...
int main(void) {
  ...
  ecc_elapsed_time = ecc_sign((uint8_t *)TEST_MESSAGE, (size_t) TEST_MESSAGE_LEN, private_key, &curve, (unsigned) TESTROUNDS);
  
  printf("Average Elapsed Time: %u microseconds", (unsigned int) ecc_elapsed_time);
  ...
  aes_elapsed_time = aes_cmac((uint8_t *)TEST_MESSAGE, (size_t) TEST_MESSAGE_LEN, aes_key, (unsigned) TESTROUNDS);
  // 
  printf("Average Elapsed Time: %u microseconds", (unsigned int) aes_elapsed_time);
